...
│  │  ┌────────┐ ┌────────┐ ┌────────┐ ┌────────┐           │    │
│  │  │信号量  │ │互斥量  │ │消息队列 │ │事件标志│           │    │
│  │  │SEM     │ │MUTEX   │ │MSG Q   │ │FLAG    │           │    │
│  │  └────────┘ └────────┘ └────────┘ └────────┘           │    │
│  └─────────────────────────────────────────────────────────┘    │
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │                    内存管理模块                           │    │
│  │         OSMalloc() / OSFree() / 内存池管理                │    │
│  └─────────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────────┘
                              │
┌─────────────────────────────▼───────────────────────────────────┐
...
│
├── SourceCode/                    # 核心源码目录
│   ├── os_cfg.h                   # 配置文件（功能裁剪开关）
│   ├── os_datatype.h              # 基础数据类型定义
│   ├── CasyOS.h                   # API 声明头文件
│   ├── CasyOS.c                   # 内核核心（异常处理/调度触发）
│   ├── os_task.c                  # 任务管理（就绪队列/位图/延时链表）
│   ├── os_sem.c                   # 信号量实现
│   ├── os_mutex.c                 # 互斥量实现
│   ├── os_q.c                     # 消息队列实现
│   ├── os_flag.c                  # 事件标志组实现
│   └── os_mem.c                   # 内存管理实现
│
├── STM32F103RCT6_KeilDemo/        # STM32F1 示例工程
├── GD32F303ZET6_KeilDemo/         # GD32F3 简易示例
├── GD32F303ZET6_KeilFullDemo/     # GD32F3 完整示例
│
└── learning-site/                 # 本地学习网站
```
...
正常运行阶段 (SysTick + PendSV):
┌─────────────────────────────────────────────────────────────┐
│  任务 A 运行                                                 │
│         │                                                   │
│         ▼                                                   │
│  ┌─────────────┐                                           │
│  │ SysTick中断 │ ◀── 每 1ms 触发一次                        │
│  │ 进入        │                                           │
│  └──────┬──────┘                                           │
│         │                                                   │
│         ▼                                                   │
│  ┌─────────────┐                                           │
│  │OS_TickUpdate│ 所有延时任务 tick 递减                      │
│  └──────┬──────┘                                           │
│         │                                                   │
│         ▼                                                   │
│  ┌─────────────┐                                           │
...
│           │        └──────┬──────┘        │                    │
│           │               │                │                    │
│    被调度器选中│               │OS_RdyTaskAdd() │ 从挂起/延时时钟唤醒   │
│           │               │                │                    │
│           │               ▼                │                    │
│           │        ┌─────────────┐        │                    │
│           │        │  RUNNING    │        │                    │
...
│         ▼                                                     │
│  ┌─────────────┐                                             │
│  │检查位图是否为 0│ ─────是─────▶ 无就绪任务 (应永不发生)       │
│  └──────┬──────┘                                             │
│         │ 否                                                  │
│         ▼                                                     │
│  ┌─────────────┐                                             │
│  │__builtin_clz│  硬件指令，单周期                             │
│  │(位图)       │                                             │
│  └──────┬──────┘                                             │
│         │                                                     │
│         ▼                                                     │
│  ┌─────────────┐                                             │
│  │计算最高优先级│  prio = 31 - clz result                     │
│  └──────┬──────┘                                             │
│         │                                                     │
│         ▼                                                     │
│  ┌─────────────┐                                             │
│  │从 RdyList   │  取该优先级队列头结点                         │
│  │取出头结点   │                                             │
│  └──────┬──────┘                                             │
│         │                                                     │
│         ▼                                                     │
│  ┌─────────────┐                                             │
│  │更新         │  g_pCurrentTask = p_tcb                     │
│  │g_pCurrentTask│                                             │
│  └──────┬──────┘                                             │
│         │                                                     │
│         ▼                                                     │
...
│  pendList:                                                  │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ head ↔ [TCB_Prio5] ↔ [TCB_Prio10] ↔ [TCB_Prio15] ↔ tail │   │
│  │      (按优先级排序插入)                                │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  当 OSSemPost() 调用时:                                     │
│  1. 从 pendList 取出 head (最高优先级等待者)                 │
│  2. 将该 TCB 移入就绪队列                                    │
│  3. 设置 state = READY                                      │
│  4. 触发调度 (可能抢占当前任务)                              │
└─────────────────────────────────────────────────────────────┘
